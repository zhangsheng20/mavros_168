...
public:
			...
			diagnostic_updater::Updater &diag_updater)
	{
        ...
        for (uint8_t i = 0; i < uav_sys_id_list_.size(); i++)
        {
            ...
            fix_pub_list_.push_back(nh.advertise<sensor_msgs::NavSatFix>("fix" + sys_id, 10));
            origin_pub_list_.push_back(nh.advertise<sensor_msgs::NavSatFix>("origin" + sys_id, 10));
            time_ref_pub_list_.push_back(nh.advertise<sensor_msgs::TimeReference>("time_reference" + sys_id, 10));
            vel_pub_list_.push_back(nh.advertise<geometry_msgs::TwistStamped>("gps_vel" + sys_id, 10));
        }
	}
	...
	const message_map get_rx_handlers() {
		return {
			...
			MESSAGE_HANDLER(MAVLINK_MSG_ID_GPS_GLOBAL_ORIGIN, &GPSPlugin::handle_gps_origin),
		};
	}
...
private:
	...
	std::vector<ros::Publisher> origin_pub_list_;
   
    void handle_gps_origin(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {

    	mavlink_gps_global_origin_t home_gps;
    	mavlink_msg_gps_global_origin_decode(msg, &home_gps);

		sensor_msgs::NavSatFixPtr origin = boost::make_shared<sensor_msgs::NavSatFix>();
		geometry_msgs::TwistStampedPtr vel = boost::make_shared<geometry_msgs::TwistStamped>();

		//gps_diag.set_gps_raw(home_gps);



		origin->latitude = home_gps.latitude/ 1E7; // deg
		origin->longitude = home_gps.longitude / 1E7; // deg
		origin->altitude = home_gps.altitude/ 1E3; // m
		origin->header.frame_id = frame_id;
		origin->header.stamp = ros::Time::now();
		publishMessage(sysid, compid, origin_pub_list_, origin);
    }
	...
	void handle_gps_raw_int(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid)
	{
        ...
        publishMessage(sysid, compid, fix_pub_list_, fix);
		...
		if (raw_gps.vel != UINT16_MAX &&
            ...
            publishMessage(sysid, compid, vel_pub_list_, vel);
		}
	}
