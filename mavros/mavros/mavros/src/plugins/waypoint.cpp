...
public:
			...
			diagnostic_updater::Updater &diag_updater)
	{
        ...
        for (unsigned int i = 0; i < uav_sys_id_list_.size(); i++)
        {
            ...
            wp_list_pub_list_.push_back(wp_nh.advertise<mavros::WaypointList>("waypoints" + sys_id, 2, true));
        }
#if 0
		...
		shedule_timer.stop();
		uas_->sig_connection_changed.connect(boost::bind(&WaypointPlugin::connection_cb, this, _1));
#endif
	};
...
private:
	...
	std::vector<ros::Publisher> wp_list_pub_list_;
	...
	void handle_mission_item(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
		...
		if (wp_state == WP_RXWP) {
			...
			if (++wp_cur_id < wp_count) {
				...
				lock.unlock();
				publish_waypoints(sysid, compid);
			}
		}
		else {
	...
	void publish_waypoints(uint8_t sysid, uint8_t compid) {
		...
		lock.unlock();
		publishMessage(sysid, compid, wp_list_pub_list_, wpl);
	}
	...
	void mission_item(WaypointItem &wp) {
		...
		mavlink_msg_mission_item_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				wp.seq,
				wp.frame,
				wp.command,
	...
	void mission_request(uint16_t seq) {
		...
		mavlink_msg_mission_request_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				seq
				);
		UAS_FCU(uas_)->send_message(&msg);
	...
	void mission_set_current(uint16_t seq) {
		...
		mavlink_msg_mission_set_current_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				seq
				);
		UAS_FCU(uas_)->send_message(&msg);
	...
	void mission_request_list() {
		...
		mavlink_msg_mission_request_list_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_)
				);
		UAS_FCU(uas_)->send_message(&msg);
	}
	...
	void mission_count(uint16_t cnt) {
		...
		mavlink_msg_mission_count_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				cnt
				);
		UAS_FCU(uas_)->send_message(&msg);
	...
	void mission_clear_all() {
		...
		mavlink_msg_mission_clear_all_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_)
				);
		UAS_FCU(uas_)->send_message(&msg);
	}
	...
	void mission_ack(enum MAV_MISSION_RESULT type) {
		...
		mavlink_msg_mission_ack_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				type
				);
		UAS_FCU(uas_)->send_message(&msg);
