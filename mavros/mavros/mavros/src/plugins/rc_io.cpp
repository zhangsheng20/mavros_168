...
public:
			...
			diagnostic_updater::Updater &diag_updater)
	{
		...
		for (unsigned int i = 0; i < uav_sys_id_list_.size(); i++)
        {
		    ...
		    rc_in_pub_list_.push_back(rc_nh.advertise<mavros::RCIn>("in" + sys_id, 10));
		    rc_out_pub_list_.push_back(rc_nh.advertise<mavros::RCOut>("out" + sys_id, 10));
		}
		override_sub = rc_nh.subscribe("override", 10, &RCIOPlugin::override_cb, this);

		uas_->sig_connection_changed.connect(boost::bind(&RCIOPlugin::connection_cb, this, _1));
	};
...
private:
	...
	std::vector<ros::Publisher> rc_in_pub_list_;
	std::vector<ros::Publisher> rc_out_pub_list_;
...
#undef SET_RC_IN
        ...
        publishMessage(sysid, compid, rc_in_pub_list_, rcin_msg);
	}
...
#undef SET_RC_OUT
        ...
        publishMessage(sysid, compid, rc_out_pub_list_, rcout_msg);
	}
	...
	void rc_channels_override(const boost::array<uint16_t, 8> &channels) {
		...
		mavlink_msg_rc_channels_override_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				channels[0],
				channels[1],
				channels[2],
