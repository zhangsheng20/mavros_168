...
public:
			...
			diagnostic_updater::Updater &diag_updater)
	{
	    for (unsigned int i = 0; i < uav_sys_id_list_.size(); i++)
        {
		    ...
		    vfr_pub_list_.push_back(nh.advertise<mavros::VFR_HUD>("vfr_hud" + sys_id, 10));

#ifdef MAVLINK_MSG_ID_WIND
		    wind_pub_list_.push_back(nh.advertise<geometry_msgs::TwistStamped>("wind_estimation" + sys_id, 10));
#endif
        }
	}
...
private:
	std::vector<ros::Publisher> vfr_pub_list_;
	std::vector<ros::Publisher> wind_pub_list_;
	...
	void handle_vfr_hud(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
		...
		publishMessage(sysid, compid, vfr_pub_list_, vmsg);
	}
	...
	void handle_wind(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
		...
		publishMessage(sysid, compid, wind_pub_list_, twist);
	}
#endif
};
