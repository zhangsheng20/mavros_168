...
public:
	...
	{	    
		...
		for (uint8_t i = 0; i < uav_sys_id_list_.size(); i++)
        {
            ...
            cmd_pub_list_.push_back(cmd_nh.advertise<mavros::Cmd>("vehicle_cmd" + sys_id,10));
        }
	}
...
private:
	...
	std::vector<ros::Publisher>  cmd_pub_list_;
	...
	void handle_command_long(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid){
		...
		mavros::Cmd vehicle_cmd;
		vehicle_cmd.param1 = cmd.param1;
		vehicle_cmd.param2 = cmd.param2;
		vehicle_cmd.param3 = cmd.param3;
		vehicle_cmd.param4 = cmd.param4;
		vehicle_cmd.param5 = cmd.param5;
		vehicle_cmd.param6 = cmd.param6;
		vehicle_cmd.param7 = cmd.param7;
		vehicle_cmd.command = cmd.command;
		vehicle_cmd.target_system = cmd.target_system;
		vehicle_cmd.target_component = cmd.target_component;
		vehicle_cmd.confirmation = cmd.confirmation;
		publishMessage(sysid, compid, cmd_pub_list_, vehicle_cmd);
	...
	bool send_command_long_and_wait(uint16_t command, uint8_t confirmation,
		...
		bool is_ack_required = (confirmation != 0 || uas_->is_ardupilotmega()) && !uas_->is_px4();
		if (is_ack_required)
	...
	void command_long(uint16_t command, uint8_t confirmation,
		...
		mavlink_msg_command_long_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				command,
				confirmation,
				param1, param2,
	...
	void command_int(uint8_t frame, uint16_t command,
		...
		mavlink_msg_command_int_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				frame,
				command,
				current,
