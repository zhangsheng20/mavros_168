...
public:
			...
			diagnostic_updater::Updater &diag_updater)
	{
		...
		timeout_timer.stop();
		uas_->sig_connection_changed.connect(boost::bind(&ParamPlugin::connection_cb, this, _1));
#endif
	}
...
private:
	...
	void param_request_list() {
		...
		mavlink_msg_param_request_list_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_)
				);
	...
	void param_request_read(std::string id, int16_t index=-1) {
		...
		mavlink_msg_param_request_read_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				param_id,
				index
				);
	...
	void param_set(Parameter &param) {
		...
		mavlink_msg_param_set_pack_chan(UAS_PACK_CHAN(uas_), &msg,
				UAS_PACK_TGT(uas_),
				param_id,
				pu.param_float,
				pu.type
