...
#include <mavros/Bucket_status.h>
...
public:
			...
			diagnostic_updater::Updater &diag_updater)
	{
		...
		for (unsigned int i = 0; i < uav_sys_id_list_.size(); i++)
        {
            ...
            imu_pub_list_.push_back(nh.advertise<sensor_msgs::Imu>("imu/data" + sys_id, 10));
            magn_pub_list_.push_back(nh.advertise<sensor_msgs::MagneticField>("imu/mag" + sys_id, 10));
            temp_pub_list_.push_back(nh.advertise<sensor_msgs::Temperature>("imu/temperature" + sys_id, 10));
            press_pub_list_.push_back(nh.advertise<sensor_msgs::FluidPressure>("imu/atm_pressure" + sys_id, 10));
            imu_raw_pub_list_.push_back(nh.advertise<sensor_msgs::Imu>("imu/data_raw" + sys_id, 10));
        }
	}
...
private:
	...
	std::vector<ros::Publisher> imu_pub_list_;
	std::vector<ros::Publisher> imu_raw_pub_list_;
	std::vector<ros::Publisher> adc_status_pub_list_;
	std::vector<ros::Publisher> magn_pub_list_;
	std::vector<ros::Publisher> temp_pub_list_;
	std::vector<ros::Publisher> press_pub_list_;
	...
	void uas_store_attitude(uint8_t sysid, uint8_t compid,
	...
	void handle_attitude(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
		...
		// NED -> ENU (body-fixed)
		// note orientation should be converted to forward left up frame in ROS standard, while position should be ENU
		...
		uas_store_attitude(sysid, compid,
		        orientation,
		...
		publishMessage(sysid, compid, imu_pub_list_, imu_msg);
	}
	...
	void handle_attitude_quaternion(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
		...
		// PX4 NED (w x y z) -> ROS NWU (x -y -z w) (body-fixed)
		// mavlink message quaternion is in order of (w,x,y,z),
		// but tf::Quaternion is in order of (x,y,z,w).
	...
	void handle_highres_imu(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid) {
		...
		if (imu_hr.fields_updated & ((7<<3)|(7<<0))) {
			...
			publishMessage(sysid, compid, imu_raw_pub_list_, imu_msg);
		}
		...
		if (imu_hr.fields_updated & (7<<6)) {
			...
			publishMessage(sysid, compid, magn_pub_list_, magn_msg);
		}
		...
		if (imu_hr.fields_updated & (1<<9)) {
			...
			publishMessage(sysid, compid, press_pub_list_, atmp_msg);
		}
		...
		if (imu_hr.fields_updated & (1<<12)) {
			...
			publishMessage(sysid, compid, temp_pub_list_, temp_msg);
		}
	}

	void handle_raw_imu(const mavlink_message_t *msg, uint8_t sysid, uint8_t compid)
	{
		if (has_hr_imu || has_scaled_imu) return;
		...
		if (!uas->is_ardupilotmega()) {
			ROS_WARN_THROTTLE_NAMED(60, "imu", "[%d, %d]RAW_IMU: linear acceleration known on APM only", sysid, compid);
