...
static bool resolve_address_udp(io_service &io, std::string host,
  ...
  std::for_each(resolver.resolve(query, ec), udp::resolver::iterator(),
                [&](const udp::endpoint &q_ep) {
                  ...
                  ep.port(port);
                  result = true;
                  CONSOLE_BRIDGE_logDebug("udp: host %s resolved as %s", host.c_str(),
                           to_string_ss(ep).c_str());
                });

  if (ec) {
    CONSOLE_BRIDGE_logWarn("udp: resolve error: %s", ec.message().c_str());
    result = false;
  }
...
MAVConnUDP::MAVConnUDP(uint8_t system_id, uint8_t component_id,
  ...
  CONSOLE_BRIDGE_logInform("udp%d: Bind address: %s", channel, to_string_ss(bind_ep).c_str());

  if (remote_host != "") {
    remote_exists =
        resolve_address_udp(io_service, remote_host, remote_port, remote_ep);

    if (remote_exists)
      CONSOLE_BRIDGE_logInform("udp%d: Remote address: %s", channel,
                to_string_ss(remote_ep).c_str());
    else
      CONSOLE_BRIDGE_logWarn("udp%d: Remote address resolve failed.", channel);
  }
...
void MAVConnUDP::send_bytes(const uint8_t *bytes, size_t length) {
  if (!is_open()) {
    CONSOLE_BRIDGE_logError("udp%d:send: channel closed!", channel);
    return;
  }

  if (!remote_exists) {
    CONSOLE_BRIDGE_logDebug("udp%d:send: Remote not known, message dropped.", channel);
    return;
  }
...
void MAVConnUDP::send_message(const mavlink_message_t *message, uint8_t sysid,
                              uint8_t compid) {
  ...
  CONSOLE_BRIDGE_logDebug("[udp%d][send] Message-Id: %d [%d bytes] Sys-Id: %d Comp-Id: %d",
...
void MAVConnUDP::do_recvfrom() {
  socket.async_receive_from(
      buffer(rx_buf, sizeof(rx_buf)), remote_ep,
      boost::bind(&MAVConnUDP::async_receive_end, this,
...
void MAVConnUDP::async_receive_end(error_code error, size_t bytes_transferred) {
  ...
  if (error) {
    CONSOLE_BRIDGE_logError("[udp%d] [receive] %s", channel, error.message().c_str());
    // close();
    return;
  }
  ...
  for (ssize_t i = 0; i < bytes_transferred; i++) {
    if (mavlink_parse_char(channel, rx_buf[i], &message, &status)) {
      CONSOLE_BRIDGE_logDebug("[udp%d][recv] Message-Id: %d [%d bytes] Sys-Id: %d Comp-Id: %d",
...
void MAVConnUDP::do_sendto(bool check_tx_state) {
  ...
  socket.async_send_to(
      buffer(buf->dpos(), buf->nbytes()), remote_ep,
      boost::bind(&MAVConnUDP::async_sendto_end, this,
                  boost::asio::placeholders::error,
                  boost::asio::placeholders::bytes_transferred));
}
...
void MAVConnUDP::async_sendto_end(error_code error, size_t bytes_transferred) {
  if (error) {
    CONSOLE_BRIDGE_logError("[udp%d] [sendto] %s", channel, error.message().c_str());
    // close();
