...
static bool resolve_address_tcp(io_service &io, std::string host,
  ...
  std::for_each(resolver.resolve(query, ec), tcp::resolver::iterator(),
                [&](const tcp::endpoint &q_ep) {
                  ...
                  ep.port(port);
                  result = true;
                  CONSOLE_BRIDGE_logDebug("tcp: host %s resolved as %s", host.c_str(),
                           to_string_ss(ep).c_str());
                });

  if (ec) {
    CONSOLE_BRIDGE_logWarn("tcp: resolve error: %s", ec.message().c_str());
    result = false;
  }
...
MAVConnTCPClient::MAVConnTCPClient(uint8_t system_id, uint8_t component_id,
  ...
  CONSOLE_BRIDGE_logInform("tcp%d: Server address: %s", channel,
...
MAVConnTCPClient::MAVConnTCPClient(uint8_t system_id, uint8_t component_id,
    ...
    : MAVConnInterface(system_id, component_id), socket(server_io) {
  // waiting when server call client_connected()
}

void MAVConnTCPClient::client_connected(int server_channel) {
  CONSOLE_BRIDGE_logInform("tcp-l%d: Got client, channel: %d, address: %s", server_channel,
            channel, to_string_ss(server_ep).c_str());

  // start recv
  socket.get_io_service().post(boost::bind(&MAVConnTCPClient::do_recv, this));
}

MAVConnTCPClient::~MAVConnTCPClient() { close(); }
...
void MAVConnTCPClient::close() {
  ...
  // clear tx queue
  std::for_each(tx_q.begin(), tx_q.end(), [](MsgBuffer *p) { delete p; });
  tx_q.clear();
...
void MAVConnTCPClient::send_bytes(const uint8_t *bytes, size_t length) {
  if (!is_open()) {
    CONSOLE_BRIDGE_logError("tcp%d:send: channel closed!", channel);
    return;
  }
  ...
  socket.get_io_service().post(
      boost::bind(&MAVConnTCPClient::do_send, this, true));
}
...
void MAVConnTCPClient::send_message(const mavlink_message_t *message,
  ...
  CONSOLE_BRIDGE_logDebug("tcp%d:send: Message-ID: %d [%d bytes] Sys-Id: %d Comp-Id: %d",
...
void MAVConnTCPClient::do_recv() {
  socket.async_receive(
      ...
      boost::bind(&MAVConnTCPClient::async_receive_end, this,
                  boost::asio::placeholders::error,
                  boost::asio::placeholders::bytes_transferred));
}

void MAVConnTCPClient::async_receive_end(error_code error,
                                         size_t bytes_transferred) {
  mavlink_message_t message;
  mavlink_status_t status;

  if (error) {
    CONSOLE_BRIDGE_logError("[tcp%d] [receive] %s", channel, error.message().c_str());
    // close();
    return;
  }
  ...
  for (ssize_t i = 0; i < bytes_transferred; i++) {
    if (mavlink_parse_char(channel, rx_buf[i], &message, &status)) {
      CONSOLE_BRIDGE_logDebug("tcp%d:recv: Message-Id: %d [%d bytes] Sys-Id: %d Comp-Id: %d",
               channel, message.msgid, message.len, message.sysid,
               message.compid);
...
void MAVConnTCPClient::do_send(bool check_tx_state) {
  ...
  socket.async_send(buffer(buf->dpos(), buf->nbytes()),
                    boost::bind(&MAVConnTCPClient::async_send_end, this,
                                boost::asio::placeholders::error,
                                boost::asio::placeholders::bytes_transferred));
}

void MAVConnTCPClient::async_send_end(error_code error,
                                      size_t bytes_transferred) {
  if (error) {
    CONSOLE_BRIDGE_logError("[tcp%d] [sendto] %s", channel, error.message().c_str());
    // close();
...
MAVConnTCPServer::MAVConnTCPServer(uint8_t system_id, uint8_t component_id,
  ...
  CONSOLE_BRIDGE_logInform("tcp-l%d: Bind address: %s", channel,
...
MAVConnTCPServer::~MAVConnTCPServer() { close(); }
...
void MAVConnTCPServer::do_accept() {
  ...
  acceptor_client =
      boost::make_shared<MAVConnTCPClient>(sys_id, comp_id, io_service);
  acceptor.async_accept(acceptor_client->socket, acceptor_client->server_ep,
                        boost::bind(&MAVConnTCPServer::async_accept_end, this,
                                    boost::asio::placeholders::error));
}

void MAVConnTCPServer::async_accept_end(error_code error) {
  if (error) {
    CONSOLE_BRIDGE_logError("tcp-l%d:accept: ", channel, error.message().c_str());
    close();
    return;
  }
  ...
  //	ROS_ERROR_NAMED("mavconn", "tcp-l:accept_cb: all channels in use, drop
  // connection");
  //	client_sock.close();
  //	return;
  //}
  ...
  acceptor_client->message_received.connect(
      boost::bind(&MAVConnTCPServer::recv_message, this, _1, _2, _3));
  acceptor_client->port_closed.connect(
      boost::bind(&MAVConnTCPServer::client_closed, this,
...
void MAVConnTCPServer::client_closed(
  ...
  if (auto instp = weak_instp.lock()) {
    ...
    CONSOLE_BRIDGE_logInform("tcp-l%d: Client connection closed, channel: %d, address: %s",
...
void MAVConnTCPServer::recv_message(const mavlink_message_t *message,
                                    uint8_t sysid, uint8_t compid) {
