...
int MAVConnInterface::new_channel() {
	...
	for (chan = 0; chan < MAVLINK_COMM_NUM_BUFFERS; chan++) {
		if (allocated_channels.count(chan) == 0) {
			CONSOLE_BRIDGE_logDebug("Allocate new channel: %d", chan);
			allocated_channels.insert(chan);
			return chan;
		}
	}

	CONSOLE_BRIDGE_logError("channel overrun");
	return -1;
}

void MAVConnInterface::delete_channel(int chan) {
	std::lock_guard<std::recursive_mutex> lock(channel_mutex);
	CONSOLE_BRIDGE_logDebug("Freeing channel: %d", chan);
		...
		uint8_t sysid, uint8_t compid)
{
	...
	if (message->sysid != sysid || message->compid != compid) {
		...
		// mavlink_finalize_message_chan(&msg, sysid, compid, channel, 0, message->len,
		// 		mavlink_crcs[msg.msgid]);
//		printf("check sum 111...................%x ",(char)(msg.checksum & 0xFF));
//		printf("check sum 222...................%x ",(char)(msg.checksum >> 8));
...
static void url_parse_query(std::string query, uint8_t &sysid, uint8_t &compid)
{
	...
	if (ids_it == query.end()) {
		CONSOLE_BRIDGE_logWarn("URL: unknown query arguments");
		return;
	}
	...
	if (comma_it == query.end()) {
		CONSOLE_BRIDGE_logError("URL: no comma in ids= query");
		return;
	}
	...
	CONSOLE_BRIDGE_logDebug("URL: found system/component id = [%u, %u]", sysid, compid);
}
		...
		uint8_t system_id, uint8_t component_id)
{
	...
	CONSOLE_BRIDGE_logDebug("[interface.cpp] opened serial %s",path.c_str());
	// /dev/ttyACM0:57600
	url_parse_host(path, file_path, baudrate, "/dev/ttyACM0", 57600);
	url_parse_query(query, system_id, component_id);

	// std::cout<<"==="<<path.c_str()<<"==="<<file_path.c_str()<<"==="<<baudrate<<std::endl;
	// std::cout<<"==="<<query.c_str()<<"==="<<int(system_id)<<"==="<<int(component_id)<<std::endl;
	// std::cout<<"==="<<int(system_id)<<"==="<<int(component_id)<<"==="<<file_path<<"==="<<baudrate<<std::endl;
		...
		uint8_t system_id, uint8_t component_id)
{
	...
	if (sep_it == hosts.end()) {
		CONSOLE_BRIDGE_logError("UDP URL should contain @!");
...
MAVConnInterface::Ptr MAVConnInterface::open_url(std::string url,
	...
	CONSOLE_BRIDGE_logDebug("[interface.cpp] opened serial %s",url.c_str());
	...
	if (proto_it == url.end()) {
		...
		CONSOLE_BRIDGE_logDebug("URL: %s: looks like file path", url.c_str());
		// std::cout<<"URL:"<< url.c_str() << "looks like file path"<<std::endl;;
		//URL:/dev/uav_01:115200
	...
	CONSOLE_BRIDGE_logDebug("URL: %s: proto: %s, host: %s, path: %s, query: %s",
			url.c_str(), proto.c_str(), host.c_str(),
			path.c_str(), query.c_str());

	// std::cout<<url.c_str()<<proto.c_str()<<host.c_str()<<path.c_str()<<query.c_str()<<std::endl;
	if (proto == "udp")
